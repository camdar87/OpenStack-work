...
def destroy():
    ...
    if floating_ip:
        ...
        try:
            ...
            try:
                conn.network.release_ip(floating_ip)
                print("Released Floating IP.")
            except openstack.exceptions.SDKException as e:
                print(f"Failed to release Floating IP: {e}")
...
def status():
    ...
    for server_name in server_names:
        ...
        if server:
            ...
            # Check if server has floating IP
            floating_ips = conn.network.ips(server=server)
            if floating_ips:
                print(f"Server '{server_name}' Floating IPs:")
                for floating_ip in floating_ips:
                    print(f"  {floating_ip.floating_ip_address}")
            else:
